...
def download_and_generate_residential():
    ...
    for row in reader_pre:
        ...
        calculator_dataset[f"{c_name}, {s_name}"] = {
            "cost": base_cost,
            "days": base_days,
            "url": f"/residential-pages/{f_name}"
        }
    ...
    for row in reader:
        ...
        for peer in state_groups[state_name]:
            if peer['city'] != city_name:
                grid_html += f'        <a href="/residential-pages/{peer["filename"]}" style="color: #10b981; text-decoration: none; font-size: 0.9rem;">• {peer["city"]} Boundary Cost</a>\n'
...
{directory_links_html}    </ul>
    ...
    # Save exactly to public/hub.html to eliminate directory name collisions
